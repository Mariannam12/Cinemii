...
// Deterministic color per username so avatars stay consistent
...
export function CommunityChat({ onClose }) {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      ...
      else setMessages(prev => [...prev.slice(-119), data]);
    };
    return () => ws.close();
  }, [name]);
  ...
  const send = useCallback(() => {
    ...
    wsRef.current.send(JSON.stringify({ type: 'chat', name, text: text.trim() }));
    setText('');
  }, [text, name, loggedIn]);
            ...
            <div key={i} className="flex items-start gap-2.5 group">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline gap-2">
                  <span className="text-sm font-semibold truncate" style={{ color: c }}>{m.name}</span>
                  <span className="text-muted/60 text-[11px] ml-auto flex-shrink-0">{fmtTime(m.ts)}</span>
                </div>
                <p className="text-white/90 text-sm leading-snug break-words mt-0.5">{m.text}</p>
              </div>
            </div>
          );
        })}
